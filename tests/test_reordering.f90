    ...
    do j = 1, ydirps%n
      do i = 1, xdirps%n
        call test_index_reversing(pass_X, i, j, k, DIR_X, &
                                  SZ, allocator%xdims_padded(2), allocator%ydims_padded(2), allocator%zdims_padded(2))
...
#else
    if (norm2(a%data(1:SZ, 1:globs%nx_loc, 1:globs%n_groups_x) - b%data(1:SZ, 1:globs%nx_loc, 1:globs%n_groups_x)) > tol) then
      allpass = .false.
      write (stderr, '(a)') message
    end if
#endif
