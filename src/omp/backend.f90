module m_omp_backend
  ...
  use m_common, only: dp, get_dirs_from_rdr, get_rdr_from_dirs, &
                      move_data_loc, DIR_X, DIR_Y, DIR_Z, DIR_C, NULL_LOC
...
contains
  ...
  subroutine field_max_mean_omp(self, max_val, mean_val, f, enforced_data_loc)
    ...
    real(dp) :: val, max_p, sum_p, mean_p, max_pncl, sum_pncl
    ...
    if (f%data_loc == NULL_LOC .and. (.not. present(enforced_data_loc))) then
      error stop 'The input field to field_max_mean does not have a valid &
                  &f%data_loc. You may enforce a data_loc of your choice &
                  &as last argument to carry on at your own risk!'
    end if
    ...
    mean_p = sum_p/product(self%mesh%get_global_dims(data_loc))

    call MPI_Reduce(max_p, max_val, 1, MPI_DOUBLE_PRECISION, MPI_MAX, 0, &
                    MPI_COMM_WORLD, ierr)
    call MPI_Reduce(mean_p, mean_val, 1, MPI_DOUBLE_PRECISION, MPI_SUM, 0, &
                    MPI_COMM_WORLD, ierr)
