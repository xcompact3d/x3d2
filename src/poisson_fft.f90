...
contains
  ...
  subroutine stretching_matrix(self, geo, xdirps, ydirps, zdirps)
    ...
    if (trim(geo%stretching(2)) == 'bottom') then
      ...
      a0 = geo%alpha(2)/pi + geo%beta(2)/(2*pi)
        ...
        do j = 1, self%ny_spec
          do i = 1, self%nx_spec
            ...
            if (iy == 1) then
              ...
              km_a1 = self%get_km(ix, self%ny_spec/2 - 1, iz)
            else
            ...
            self%a_re(i, j, k, 3) = &
              ...
              - (get_real(self%kz(iz)) &
                 *self%trans_y_re(iy)*self%trans_x_re(iz))**2 &
            ...
            self%a_im(i, j, k, 3) = &
              ...
              - (get_imag(self%kz(iz)) &
                 *self%trans_y_im(iy)*self%trans_x_im(iz))**2 &
      ...
      do k = 1, self%nz_spec
        do j = 1, self%ny_spec - 2
          do i = 1, self%nx_spec
            ...
            self%a_re(i, j, k, 5) = &
              -a1*a1*self%get_km_re(ix, iy + 1, iz) &
              *self%get_km_re(ix, iy + 2, iz)
            self%a_im(i, j, k, 5) = &
              -a1*a1*self%get_km_im(ix, iy + 1, iz) &
