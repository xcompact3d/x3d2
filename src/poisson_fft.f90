module m_poisson_fft
  ...
  use m_common, only: dp, pi, CELL, BC_PERIODIC
...
contains
  ...
  subroutine base_init(self, mesh, xdirps, ydirps, zdirps)
    ...
    self%periodic_x = all(mesh%BCs_global(1, :) == BC_PERIODIC)
    self%periodic_y = all(mesh%BCs_global(2, :) == BC_PERIODIC)
    self%periodic_z = all(mesh%BCs_global(3, :) == BC_PERIODIC)
  ...
  subroutine wave_numbers(a, b, k, e, k2, n, L, d, periodic, c_a, c_b, c_alpha)
    ...
    complex(dp), dimension(:), intent(out) :: a, b
